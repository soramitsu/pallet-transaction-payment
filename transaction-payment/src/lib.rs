...
//!   - A handler trait `OnTransactionPayment` for the fee distribution logic.
...
use frame_support::{
	...
	traits::{Currency, ExistenceRequirement, Get, Imbalance, WithdrawReason},
	weights::{
...
use sp_runtime::{
	traits::{
		CheckedDiv, Convert, DispatchInfoOf, Dispatchable, PostDispatchInfoOf,
		SaturatedConversion, Saturating, SignedExtension, Zero,
	},
	...
	M: Get<Multiplier>,
{
	fn convert(previous: Multiplier) -> Multiplier {
		...
		let diff =
			Multiplier::saturating_from_rational(diff_abs, normal_max_weight.max(1));
		...
		if positive {
			...
			let negative =
				first_term.saturating_sub(second_term).saturating_mul(previous);
...
pub trait OnTransactionPayment<AccountId, Imbalance, Balance>
where
	Imbalance: frame_support::traits::Imbalance<Balance>,
{
	fn on_payment(from_account: AccountId, fee: Imbalance, tip: Imbalance);
}

impl<AccountId, Imbalance, Balance> OnTransactionPayment<AccountId, Imbalance, Balance>
	for ()
where
	Imbalance: frame_support::traits::Imbalance<Balance>,
{
	fn on_payment(_from_account: AccountId, _fee: Imbalance, _tip: Imbalance) {}
}
	...
	BalanceOf<T>: FixedPointOperand,
{
	...
	fn compute_fee_raw(
		...
		if pays_fee == Pays::Yes {
			...
			let multiplier_numerator: BalanceOf<T> =
				multiplier.into_inner().saturated_into();
	...
	fn weight_to_fee(weight: Weight) -> BalanceOf<T> {
		...
		let capped_weight =
			weight.min(<T as frame_system::Trait>::MaximumBlockWeight::get());
	...
	T::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
	...
	type Pre =
		(BalanceOf<T>, Self::AccountId, Option<NegativeImbalanceOf<T>>, BalanceOf<T>);
	...
	fn post_dispatch(
		...
		if let Some(payed) = imbalance {
			let actual_fee =
				Module::<T>::compute_actual_fee(len as u32, info, post_info, tip);
			...
			T::OnTransactionPayment::on_payment(who, fee, tip);
		}
		Ok(())
	}
}
		...
		ExtBuilder::default().balance_factor(1000).byte_fee(0).build().execute_with(
			|| {
				...
				assert_eq!(
					...
					(10000 - <Runtime as frame_system::Trait>::MaximumBlockWeight::get())
						as u128
				);
			},
		);
	...
	fn refund_does_not_recreate_account() {
		ExtBuilder::default().balance_factor(10).base_weight(5).build().execute_with(
			|| {
				...
				assert!(Balances::transfer(Some(2).into(), 3, Balances::free_balance(2))
					.is_ok());
				...
				assert!(System::events().iter().any(|event| {
					event.event
						== Event::pallet_balances(pallet_balances::RawEvent::Transfer(
							2, 3, 80,
						))
				}));
				// Killed Event
	...
	fn refund_consistent_with_actual_weight() {
		ExtBuilder::default().balance_factor(10).base_weight(7).build().execute_with(
			|| {
				...
				let actual_fee = Module::<Runtime>::compute_actual_fee(
