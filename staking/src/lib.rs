...
pub type MultiCurrencyBalanceOf<T> = <<T as Config>::MultiCurrency as MultiCurrency<<T as frame_system::Config>::AccountId>>::Balance;
...
decl_event!(
	pub enum Event<T> where Balance = BalanceOf<T>, MultiCurrencyBalance = MultiCurrencyBalanceOf<T>, <T as frame_system::Config>::AccountId {
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		/// # </weight>
		#[weight = T::WeightInfo::payout_stakers()]
...
impl<T: Config> ValBurnedNotifier<MultiCurrencyBalanceOf<T>> for Module<T> {
