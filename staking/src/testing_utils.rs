...
pub fn create_validators<T: Config>(
	...
	for i in 0 .. max {
		let (stash, controller) = create_stash_controller::<T>(i, balance_factor, RewardDestination::Stash)?;
...
pub fn create_validators_with_nominators_for_era<T: Config>(
	...
	for i in 0 .. validators {
		...
		let (v_stash, v_controller) = create_stash_controller::<T>(i, balance_factor, RewardDestination::Stash)?;
		let validator_prefs = ValidatorPrefs {
			commission: Perbill::from_percent(50),
			.. Default::default()
		};
