...
pub fn create_validator_with_nominators<T: Config>(
	...
	// Create reward pool
	let total_payout: MultiCurrencyBalanceOf<T> = upper_bound.into();
	let total_payout = total_payout.saturating_mul(1000u32.into());
...
benchmarks! {
	...
	payout_all {
		...
		// Create reward pool
		let total_payout: MultiCurrencyBalanceOf<T> = 1000u32.into();
		<ErasValidatorReward<T>>::insert(current_era, total_payout);
