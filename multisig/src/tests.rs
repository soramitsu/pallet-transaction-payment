...
use frame_support::{
	assert_ok, assert_err, assert_noop, impl_outer_origin, parameter_types,
	impl_outer_dispatch, weights::Weight, impl_outer_event, traits::Filter,
...
fn multisig_only_multisig_can_add_or_remove_signatory() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(
			Multisig::add_signatory(Origin::signed(1), 2),
			Error::<Test>::UnknownMultisigAccount
		);
		assert_err!(
			Multisig::remove_signatory(Origin::signed(1), 2),
			Error::<Test>::UnknownMultisigAccount
		);
	});
}
